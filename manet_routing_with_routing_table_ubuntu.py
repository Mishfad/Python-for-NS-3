...
def IsNodeinTemp(ntemp,node):
	for index in range(len(ntemp)):
		if (ntemp[index]==node)&(node!=dest_node[0]):
			return 1
	return 0
...
# checks if this node is in the route, not destination
		if (IsNodeinTemp(ntemp,index_node1)):
#			print "Node: "+str(index_node1)+" ntemp: "+str(ntemp)+str(str1)
			pl.scatter(x_plot[str1],y_plot[str1],s=cache_size[index2,str1]*30,c='m')
...
#    while (index_route<len(track_col_seq)-2)|(index_mob<len(time_plot)):
	...
	while (time_plot[index_mob]<RouteTablewithSeq_time[index_route]):
        ...
        ntemp=[]
        print range(3,track_col_seq[index_route]+1),RouteTablewithSeq[index_route]
        print RouteTablewithSeq[index_route][:track_col_seq[index_route]+1]
...
#	cache_matrix[0,RouteTablewithSeq[0][:],0]=1
	for time_index in range(2,len(RouteTablewithSeq_time)):
...
#print "RouteTablewithSeq",RouteTablewithSeq
#cache_packets(probability p)
cache_size=cache_packets(1)
print cache_size
ManetPlotwithSeq()
