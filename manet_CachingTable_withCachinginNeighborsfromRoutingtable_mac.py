...
#    print len(RouteTablewithSeq_time)
    plot_len=40
    for index_time in range(plot_len):
	vals=cache_matrix[index_time,:,:plot_len+1]

#    fig = pl.figure(figsize=(15,8))
#    ax = fig.add_subplot(111, frameon=True, xticks=[], yticks=[])
#	print vals.shape
    	the_table=pl.table(cellText=vals, rowLabels=df.index, colLabels=df.columns, colWidths = [0.02]*vals.shape[1], loc='center', cellColours=pl.cm.hot(normal(vals)))
	the_table.alpha=0
...
#    	pl.show()
	pl.axis('off')
	pl.savefig("table_1."+str(index_time)+".png")
#	pl.pause(1)
	pl.clf()
